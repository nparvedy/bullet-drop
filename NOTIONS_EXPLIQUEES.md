...
| **2026-09-14** | **Repères Globaux vs Locaux & `top_level`** | Physique & Coordonnées | Un nœud hors de l'arbre (`avant add_child`) n'a pas de parent : `global_position` équivaut à `position` locale. Si on l'ajoute ensuite à un parent décalé (ex: `Room` à `(700, 500)`), son offset s'additionne et le propulse hors map. Règle : faire `add_child()` avant d'affecter `global_position`, ou utiliser `top_level = true` (ignorer la transformation du parent). |
| **2026-09-14** | **Setters GDScript (`set(value)`) & Cycle `_ready()`** | GDScript & Réactivité | `_ready()` n'est appelé qu'une seule fois lors du `add_child()`. Si une propriété (ex: `can_shoot`) est modifiée après instanciation, `_ready()` ne se relance pas. Un setter `set(value): can_shoot = value; _update_appearance()` garantit la réactivité visuelle immédiate. |
| **2026-09-14** | **Flushing Queries & Spawning Différé (`call_deferred`)** | Physique 2D & Cycle de vie | Le moteur physique C++ interdit d'insérer ou d'activer des objets physiques (`Area2D`, `CollisionShape2D`) en plein milieu d'une collision (`flushing_queries`). Pour faire spawner des drops/balles à l'impact ou à la mort sans erreur, on reporte l'exécution en fin de frame physique avec `ma_fonction.call_deferred(args)`. |
| **2026-09-14** | **Avertissements GDScript (Warnings) & `@warning_ignore`** | Outils & Typage / Debug | Godot possède un linter statique intégré (analogie : ESLint / TS Compiler). Si un signal est déclaré dans un singleton (`EventBus`) mais émis par d'autres nœuds, Godot s'inquiète qu'il soit inutilisé localement (`unused_signal`). L'annotation `@warning_ignore("unused_signal")` informe le compilateur que c'est un comportement intentionnel. |
| **2026-09-14** | **Division Entière en GDScript (`integer_division`)** | GDScript & Mathématiques | En GDScript, diviser deux `int` tronque immédiatement la partie décimale (ex: `5 / 2 = 2` et non `2.5`). Godot lève un warning préventif pour éviter les erreurs de calcul de ratio/taux. Pour résoudre proprement : convertir explicitement en `float` ou documenter l'opération entière. |
| **2026-09-14** | **Détection Spatiale : Polling (`_process`) vs Triggers Réactifs (`Area2D`)** | Architecture & Physique 2D | Vérifier manuellement les coordonnées d'un nœud chaque frame dans `_process` est un anti-pattern coûteux et rigide (analogie web: polling `setInterval` ou écouteur de scroll lourd). Godot fournit les `Area2D` avec signaux `body_entered` : détection événementielle passive gérée par le moteur physique C++ (analogie : `IntersectionObserver`). |
| **2026-09-14** | **Conception Spatiale Direction-Agnostique (Vecteurs vs Axes Fixes)** | Mathématiques 2D & Game Design | Coder des transitions ou des seuils en dur sur un axe fixe (ex: `pos.x + 80`) bloque l'architecture sur un seul sens de défilement. L'utilisation de vecteurs unitaires normés (`Vector2`) et de directions paramétrables (`entry_direction`) rend le système universel (donjons horizontaux, verticaux ou multi-angles). |
